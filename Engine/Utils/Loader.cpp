    ...
    while (std::getline(stream, line))
    {
        ...
        case ('f'):
            ...
            for (size_t i = 0; i < 2; i++)
            {
                ...
                result[i][0] = std::stoi(val[0]);
                result[i][1] = std::stoi(val[1]);
                result[i][2] = std::stoi(val[2]);
            }
...
namespace
{
    void processLine(char** storage, size_t offset, char separation, std::string line)
    {
